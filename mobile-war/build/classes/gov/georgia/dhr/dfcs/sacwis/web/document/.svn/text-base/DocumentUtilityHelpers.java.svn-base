...
public class DocumentUtilityHelpers {
  ...
  public static DocumentMetaData completeDocumentMetaData(DocumentSave documentSave, HttpServletRequest request,
    ...
    if (request.getAttribute("userId") != null) {
      GrndsTrace.msg(TRACE_TAG, 7, "Found userId");
      String tempString = (String) request.getAttribute("userId");
      if (!"".equals(tempString)) {
        int tempInt = Integer.parseInt((String) request.getAttribute("userId"));
        documentMetaData.setUserId(tempInt);
      }
      GrndsTrace.msg(TRACE_TAG, 7, "End set userId stage");
    }
    
    if (request.getAttribute("stageId") != null) {
      GrndsTrace.msg(TRACE_TAG, 7, "Found stageId");
      String tempString = (String) request.getAttribute("stageId");
      if (!"".equals(tempString)) {
        int tempInt = Integer.parseInt((String) request.getAttribute("stageId"));
        documentMetaData.setStageId(tempInt);
      }
      GrndsTrace.msg(TRACE_TAG, 7, "End set stageId stage");
    }
    
    if (request.getAttribute("caseId") != null) {
      GrndsTrace.msg(TRACE_TAG, 7, "Found stageId");
      String tempString = (String) request.getAttribute("caseId");
      if (!"".equals(tempString)) {
        int tempInt = Integer.parseInt((String) request.getAttribute("caseId"));
        documentMetaData.setCaseId(tempInt);
      }
      GrndsTrace.msg(TRACE_TAG, 7, "End set caseId stage");
    }
