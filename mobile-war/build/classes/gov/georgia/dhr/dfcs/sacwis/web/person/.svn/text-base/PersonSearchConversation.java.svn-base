...
public class PersonSearchConversation extends BaseHiddenFieldStateConversation {
  ...
  public void displayPersonIdentSearch_xa(GrndsExchangeContext context) {
  ...
  private PersonSearchInRec populatePersonSearchInRec(GrndsExchangeContext context,
    ...
    // Added for Mobile
    String hdnMobileIdSearch = ContextHelper.getStringSafe(context, "hdnMobileIdSearch");
    ...
    if ("Y".equals(indAddlParams) || "Y".equals(hdnMobileIdSearch)) {
...
/**
 * displayPersonSearchAddress: This method is called by clicking the address displayed in the PersonSearch
 */
  	public void displayPersonSearchAddress_xa(GrndsExchangeContext context) {

	  	PerformanceTrace performanceTrace = new PerformanceTrace(TRACE_TAG, ".displayPersonSearchAddress)");
	  	performanceTrace.enterScope();
	  	//created method for testing
	  	HttpServletRequest request = context.getRequest();
	  	BaseSessionStateManager state = getSessionStateManager(context);
	  	
	    String name = ContextHelper.getStringSafe(context, "hdnFullName");
	    //state.setAttribute("hdnFullName", name, request);
	    String address = ContextHelper.getStringSafe(context, "hdnAddress");
	    //state.setAttribute("hdnAddress", address, request);
  	}
