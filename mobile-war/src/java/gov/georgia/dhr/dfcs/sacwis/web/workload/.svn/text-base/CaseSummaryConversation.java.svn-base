...
public class CaseSummaryConversation extends BaseHiddenFieldStateConversation {
  ...
  public void displayCaseSummary_xa(GrndsExchangeContext context) {
    ...
    try {
      ...
      CCMN37SO ccmn37so = null;
      try {
    	  ccmn37so = admin.retrieveCaseSummary(ccmn37si);
    	  
      } catch (ServiceException se) {
    	  if(Messages.SQL_NOT_FOUND == se.getErrorCode()) {
    		  haveCaseSummary = false;
    	  } else {
    		  throw se;
    	  }
      }
    
      CCMN81SO ccmn81so = null;
      CCFC39SO ccfc39so = null;
      CCFC21SO ccfc21so = null;
      ROWCCMN37SOG01 caseSummary = null;
      if(haveCaseSummary) {
	      ...
	      ccmn81so = admin.findCapsCase(ccmn81si);
	      ...
	      ccfc39so = admin.findCaseMergeList(ccfc39si);
	      ...
	      ccfc21so = admin.retrieveCaseFileManagement(ccfc21si);
...
//      }
    ...
    try {
      ...
      try {
    	...
    	for (int i = 0; i < rowccmn37sog02array.length; i++) {
          String checkboxValue = CheckboxHelper.getCheckboxValue(request, "cbStageValidated_" + i);
    	...
    	admin.updateSummaryVerifiedStages(saveStageVerifiedListSI);
  ...
  public void locatingInformation(GrndsExchangeContext context) {
    ...
    try {
      ...
      if (returnBean != null) {
        // SIR#22960 Reversed the if condition above and commented out
        // {
