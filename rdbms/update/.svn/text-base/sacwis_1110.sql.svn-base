...
CONSTRAINT FK_UAS_AUDIT_ID_PERSON FOREIGN KEY (ID_PERSON_UPDATE) REFERENCES CAPS.PERSON_ENC(ID_PERSON)
) tablespace data01;
...
:NEW.DT_LAST_UPDATE := sysdate;
  if :NEW.ID_UAS_PRG_CODE_MTNT_AUDIT=0 then
    SELECT CAPS.SEQ_UAS_PRG_CODE_MTNT_AUDIT.NEXTVAL INTO dummy  FROM DUAL;
...
:NEW.DT_LAST_UPDATE := sysdate;
  if :NEW.ID_UAS_ENT_CODE_MTNT_AUDIT=0 then
    SELECT CAPS.SEQ_UAS_ENT_CODE_MTNT_AUDIT.NEXTVAL INTO dummy  FROM DUAL;
