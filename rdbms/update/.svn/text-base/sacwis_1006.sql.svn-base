--STGAP00016126 - Release(4.2) MR-075  Update TUBR_CAPS_RESOURCE trigger

-- MR-075  Update TUBR_CAPS_RESOURCE trigger to set previous reimbursability end date
-- to 1ms prior to new reimbursability effective date
...
END IF;
  ...
  IF NVL(:NEW.IND_RSRC_WRITE_HIST,'N') = 'Y' THEN
    ...
    IF (:NEW.CD_RSRC_FA_HOME_STATUS IN ('AFA','ASA','FLG','FSG')) THEN
      ...
      :NEW.IND_HOME_IVE_REIMBURSABLE := 'Y';
    ...
    ELSIF (:NEW.CD_RSRC_FA_HOME_STATUS IN ('APP','AUN','CSD','INQ','PUN','WTL')) THEN
      ...
      --WTL  Waitlist
      :NEW.IND_HOME_IVE_REIMBURSABLE := 'N';
      ...
      ELSE
        ...
        -- Unless the new status is ('CSD', 'INQ', 'APP', 'WTL') then set effective date to the day after current date
        IF :NEW.CD_RSRC_FA_HOME_STATUS IN ('CSD', 'INQ', 'APP', 'WTL') THEN
          :NEW.DT_REIMBURSABLE_EFFECTIVE := TRUNC(SYSDATE + 1);
        END IF;
    ...
    ELSIF (:NEW.CD_RSRC_FA_HOME_STATUS IN ('PFA', 'PFG', 'PSA', 'PSG', 'PCL')) THEN
      ...
      --PCL  Pending Closure
      :NEW.IND_HOME_IVE_REIMBURSABLE := :OLD.IND_HOME_IVE_REIMBURSABLE;
    ...
    -- Invalidate any previous history IV-E status by setting end date to be the latest
    -- of the new record effective date minus 1 ms or old effective date.
    -- This is to prevent issue with FAD stage displaying twice on assign workload page
    -- and also to make sure there are no reimbursable status period overlap.      
    UPDATE RESOURCE_HISTORY  RH
    SET    RH.DT_REIMBURSABLE_END           = GREATEST((:NEW.DT_REIMBURSABLE_EFFECTIVE - 1/60/60/24), RH.DT_REIMBURSABLE_EFFECTIVE)
    WHERE  RH.ID_RESOURCE                   = :NEW.ID_RESOURCE
      -- this condition takes care of previous history IVE status that started on or before the new effective date
      -- but ended beyond the new effective date
      -- therefore we need to end it on the new effective date, this is the general scenario
    AND    ((RH.DT_REIMBURSABLE_EFFECTIVE   <= :NEW.DT_REIMBURSABLE_EFFECTIVE
        AND    RH.DT_REIMBURSABLE_END       >  :NEW.DT_REIMBURSABLE_EFFECTIVE)
      -- this condition takes care of previous history IVE status that started today or earlier
      -- and is effective until a future date, therefore need to end it today. 
      -- (e.g. Closed home in the middle of current month and changing to another status with same month)
      OR   (RH.DT_REIMBURSABLE_EFFECTIVE    <= TRUNC(SYSDATE)
        AND    RH.DT_REIMBURSABLE_END       >  TRUNC(SYSDATE))
      -- this condition takes care of previous history IVE status that started after the new effective date
      -- and had an effective period beyond the new effective date that may or may not have already past,
      -- therefore need to invalidate it
      -- (e.g. AFA -> PCL -> CSD -> INQ -> CSD -> INQ -> APP -> PFA -> AFA each status at a later date in same month
      OR   (RH.DT_REIMBURSABLE_EFFECTIVE    >  :NEW.DT_REIMBURSABLE_EFFECTIVE
        AND    RH.DT_REIMBURSABLE_END       >  RH.DT_REIMBURSABLE_EFFECTIVE))
      -- exclude the history record that has the current IV-E status, will update next
    AND    RH.DT_REIMBURSABLE_END           <> to_date('12/31/4712', 'MM/DD/YYYY');
     
  --dbms_output.put_line('0');
  -- 1st: set END DATE of previous record:
  -- 2nd: SMS#97850: MR-075 Set RH.DT_REIMBURSABLE_END of previous record to the greater
  --      of RH.DT_REIMBURSABLE_EFFECTIVE or :NEW.DT_REIMBURSABLE_EFFECTIVE minus 1 ms.
  ...
  SET    RH.DT_RSHS_END            = :NEW.DT_LAST_UPDATE,
         RH.DT_REIMBURSABLE_END    = NVL2(RH.DT_REIMBURSABLE_EFFECTIVE, 
                                          GREATEST(RH.DT_REIMBURSABLE_EFFECTIVE, (:NEW.DT_REIMBURSABLE_EFFECTIVE - 1/60/60/24)),  
                                          NULL)
  WHERE  RH.ID_RESOURCE            = :NEW.ID_RESOURCE
  ...
  INSERT INTO RESOURCE_HISTORY (
    ...
    :NEW.IND_HOME_IVE_REIMBURSABLE,
    :NEW.DT_REIMBURSABLE_EFFECTIVE,
    :NEW.DT_REIMBURSABLE_END,
    :NEW.IND_HOLD_PLACEMENTS
  );
  END IF;
...
insert into caps.schema_version(id_schema_version,application_version,comments)
            values (1007, 'SacwisRev4', 'Release 4.2 - DBCR 16126');
