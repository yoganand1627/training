--STGAP00015465 - Update SQR_PIP18_PARENT_ONG_COUNTS (new codes)

--To update the view used by PIP report: Parent Without Involvement in Case Planning Status - ONG with new parent codes: LM and BM (legal and bio mother).
...
SELECT DISTINCT
    ...
    , (SELECT  (count( distinct STAGE_ONG.ID_STAGE ))  
      ...
      STAGE_ONG.CD_STAGE = 'FPR' AND  
      (STAGE_ONG.DT_STAGE_CLOSE IS NULL  OR  STAGE_ONG.DT_STAGE_CLOSE >= to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy')) AND  
      months_between(last_day(to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy'))+1,   STAGE_ONG.DT_STAGE_START ) >= 2 AND  
      STAGE_PERSON_LINK_CM.CD_STAGE_PERS_ROLE IN ('HP','PR') AND  
      STAGE_PERSON_LINK_CHILD.CD_STAGE_PERS_TYPE = 'PRN' AND  
      PERSON_CHILD.DT_PERSON_BIRTH < last_day(to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy')) AND  
      ADD_MONTHS( PERSON_CHILD.DT_PERSON_BIRTH , 12*18)  >=  to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy') AND  
      (PERSON_CHILD.DT_PERSON_DEATH IS NULL  OR  PERSON_CHILD.DT_PERSON_DEATH  >= last_day(to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy'))) AND  
      ...
      (PERSON_PARENT.DT_PERSON_DEATH IS NULL  OR  PERSON_PARENT.DT_PERSON_DEATH >= last_day(to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy'))) AND  
      (STAGE_PERSON_LINK_PARENT.CD_STAGE_PERS_REL_INT IN ('PK') OR  
      (SELECT DISTINCT (1) FROM  RELATIONSHIP RELATIONSHIP_PARENT 
        WHERE RELATIONSHIP_PARENT.CD_PERSON_RELATIONSHIP IN ('SC','BF','LF','PF','LM','BM') AND  
    ...
    , (SELECT  (count( distinct STAGE_ONG.ID_STAGE ))  
      ...
      PERSON_PARENT.DT_PERSON_DEATH >= last_day(to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy'))) AND  
      (STAGE_PERSON_LINK_PARENT.CD_STAGE_PERS_REL_INT IN ('PK') OR  
        (SELECT DISTINCT (1) FROM  RELATIONSHIP RELATIONSHIP_PARENT WHERE 
        RELATIONSHIP_PARENT.CD_PERSON_RELATIONSHIP IN ('SC','BF','LF','PF','LM','BM') AND  
        (RELATIONSHIP_PARENT.DT_RELATIONSHIP_END IS NULL  OR  
        RELATIONSHIP_PARENT.DT_RELATIONSHIP_END > sysdate) AND  
        RELATIONSHIP_PARENT.ID_PERSON = PERSON_CHILD.ID_PERSON AND 
        RELATIONSHIP_PARENT.ID_RELATED_PERSON = PERSON_PARENT.ID_PERSON) IS NOT NULL ) AND  
      STAGE_PERSON_LINK_PARENT.ID_PERSON NOT IN (SELECT  PPT_PARTICIPANT_PARENT.ID_PERSON  FROM  PPT_PARTICIPANT PPT_PARTICIPANT_PARENT,  EVENT EVENT_PPT,  PPT PPT_PARENT 
      	WHERE PPT_PARTICIPANT_PARENT.ID_EVENT = EVENT_PPT.ID_EVENT
          AND  EVENT_PPT.ID_EVENT = PPT_PARENT.ID_PPT_EVENT
          AND  PPT_PARENT.DT_PPT_DATE < last_day(to_date((select textparm from  report_work where key='dt_report'), 'mm/yyyy'))+1 
          AND  PPT_PARTICIPANT_PARENT.DT_PPT_PART < last_day(to_date((select textparm from  report_work where key='dt_report'), 'mm/yyyy'))+1 
          AND PPT_PARENT.DT_PPT_DATE >= add_months(to_date((select textparm from  report_work where key='dt_report'), 'mm/yyyy') , -5) 
          AND  EVENT_PPT.ID_EVENT_STAGE = STAGE_ONG.ID_STAGE 
          AND  PPT_PARTICIPANT_PARENT.ID_PERSON = STAGE_PERSON_LINK_PARENT.ID_PERSON) AND 
      STAGE_PERSON_LINK_PARENT.ID_PERSON NOT IN (SELECT  EVENT_PERSON_LINK_CONTACT.ID_PERSON  FROM  CONTACT CONTACT_PARENT,  EVENT_PERSON_LINK EVENT_PERSON_LINK_CONTACT
          ...
          AND  CONTACT_PARENT.DT_CONTACT_OCCURRED < last_day(to_date((select textparm from  report_work where key='dt_report'), 'mm/yyyy'))+1 
          AND  CONTACT_PARENT.DT_CONTACT_OCCURRED >= add_months(to_date((select textparm from  report_work where key='dt_report'), 'mm/yyyy'), -5 ) 
          AND  (EXISTS (SELECT 1 FROM CONTACT_CBX CBX_PUR WHERE CBX_PUR.ID_CONTACT_EVENT=CONTACT_PARENT.ID_EVENT AND CBX_PUR.CD_CBX_CODE_TYPE = 'CCNTPURP' AND CBX_PUR.CD_CONTACT_CBX IN ('CPL','JCPL'))) AND  CONTACT_PARENT.ID_CONTACT_STAGE = STAGE_ONG.ID_STAGE) AND 
      STAGE_ONG.CD_STAGE_CNTY = STAGE_ONG_M.CD_STAGE_CNTY AND  
      STAGE_ONG.CD_STAGE_REGION = STAGE_ONG_M.CD_STAGE_REGION AND  
      STAGE_ONG.ID_UNIT = STAGE_ONG_M.ID_UNIT AND  
...
insert into caps.schema_version(id_schema_version,application_version,comments)
            values (556, 'SacwisRev3', 'Release 3.3 - DBCR 15465');
