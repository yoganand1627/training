...
ALTER TABLE CAPS.PERSON_DTL MODIFY(CD_PERSON_BIRTH_COUNTY  VARCHAR2(20))
;
ALTER TABLE CAPS.RESOURCE_HISTORY ADD IND_AFTER_HOURS VARCHAR2(1)     NULL
;
ALTER TABLE CAPS.RESOURCE_HISTORY_AUDIT ADD IND_AFTER_HOURS VARCHAR2(1)     NULL
;
ALTER TABLE CAPS.CAPS_RESOURCE ADD IND_AFTER_HOURS VARCHAR2(1)     NULL
;
...
:NEW.DT_LAST_UPDATE := SYSDATE;
   ...
   IF TO_DATE(:NEW.DT_TERMINATE,'MM/DD/YYYY') <
                 LEAST(TO_DATE(:OLD.DT_END,'MM/DD/YYYY'),
                 LEAST(TO_DATE(NVL(:OLD.DT_TERMINATE,:OLD.DT_END),'MM/DD/YYYY'),
               ...
               INSERT INTO ADJUSTMENT_REQUEST (
                   ...
                   0,
                   :OLD.ID_POC_EVENT,
                   TO_DATE(:NEW.DT_TERMINATE,'MM/DD/YYYY'),
                   LEAST(TO_DATE(:OLD.DT_END,'MM/DD/YYYY'),
                   LEAST(TO_DATE(NVL(:OLD.DT_TERMINATE,:OLD.DT_END),'MM/DD/YYYY'),
                   TO_DATE(SYSDATE,'MM/DD/YYYY'))),
                   (SELECT ID_PERSON FROM EVENT_PERSON_LINK
                    WHERE ID_EVENT = :OLD.ID_POC_EVENT),
                   'POC'
               );
   END IF;
END;
...
-- BEGINNING OF SACWISIFC STUFF
-- Sequence Alter SQL

CREATE SEQUENCE SACWISIFC.SEQ_CPRS_OUTBOUND
    START WITH 1
    INCREMENT BY 1
    NOMINVALUE
    NOMAXVALUE
    ...
    NOORDER
;
GRANT SELECT ON SACWISIFC.SEQ_CPRS_OUTBOUND TO CAPS
;
GRANT SELECT ON SACWISIFC.SEQ_CPRS_OUTBOUND TO CAPSBAT
;
GRANT SELECT ON SACWISIFC.SEQ_CPRS_OUTBOUND TO CAPSON
;
GRANT SELECT ON SACWISIFC.SEQ_CPRS_OUTBOUND TO SACWISIFC
;
...
CREATE INDEX SACWISIFC.IND_CPRS_OUTBOUND1
    ON SACWISIFC.CPRS_OUTBOUND(INTERFACE_STATUS,CD_COUNTY)
TABLESPACE INDEX01
STORAGE(BUFFER_POOL DEFAULT)
NOPARALLEL
NOCOMPRESS
...
-- NOW the synonyms for SACWISIFC to the other schemas
create synonym CAPS.CPRS_OUTBOUND for SACWISIFC.CPRS_OUTBOUND;
create synonym CAPS.SEQ_CPRS_OUTBOUND for SACWISIFC.SEQ_CPRS_OUTBOUND;
...
-- change STGAP00002702
INSERT INTO caps.codes_tables
(code_type, code, DECODE, dt_last_update)
VALUES('CSECATTR','PB','Maintain State Office (99)',SYSDATE);

UPDATE caps.codes_tables
SET DECODE = 'FA Home Search'
WHERE code = 'EH'
AND code_type = 'CSECATTR';

update caps.codes_Tables
set dt_end = sysdate
where code in ('03A','03B')
and code_type = 'CREGDIV';

-- change STGAP00002762
INSERT INTO CAPS.CODES_TABLES(CODE_TYPE, CODE, DECODE) VALUES('CPLMNTYP', 'NRP', 'Non-Relative Paid');

-- change STGAP00002769
update caps.message
set txt_message = 'Unit and Reg/Div values are required for DFCS Storage Locations.'
where nbr_message = '25368';


insert into caps.schema_version (id_schema_version, application_version, comments)
                         values (177, 'SacwisRev2', 'static updates, new update trigger for payment_of_care');
