--STGAP00016979 - Release(4.4) Updates to Legal Status View for AFCARS Changes

--This DBCR is an update (newer version) of DBCR 16247.
...
--2) There will be a new view called AFCARS FOSTER CARE EPISODES. This view derives from the LEGAL STATUS VIEW and is very similar in design to the existing FOSTER CARE EPISODES view used for LENSES.. However, we have added some additional columns to look up values that will be used by the AFCARS logic
-- to exclude children from AFCARS if they were in care less then 24 hrs or only placed in a hospital or locked facility. We created a new view for these elements to avoid performance impact on the existing legal status view to calculate this information.
...
select
  ...
  from (
    ...
    ( SELECT MAX (cnsrvtrshp_removal.ID_REMOVAL_EVENT)
      ...
      AND trunc(cnsrvtrshp_removal.DT_REMOVAL) <= legal_status_episodes.custody_end
    )removal_event_id,           
      (case 
      when 
      (SELECT COUNT (*)
...
insert into caps.schema_version(id_schema_version,application_version,comments)
            values (1062, 'SacwisRev4', 'Release 4.4 - DBCR 16979');
