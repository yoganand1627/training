--STGAP00014579 - UnitEmplink trigger: exclude SE

--Through code review for another problem, it was found that the TUBR trigger on Unit Emp Link table updates workload and stage id unit when the SE on the stage changes unit. The trigger should only update workload id unit and stage id unit when the PR on the stage changes unit with the PR's new unit id.

--In the revised trigger there are 2 single lines added, one to each update workload and update stage statement. Added lines has comment containing this CQ#.

-- Bogus grant statement below to make the script work with ant script and SQL Plus
...
BEGIN
        ...
        THEN
                ...
                IF (  (:NEW.ID_UNIT <> :OLD.ID_UNIT)  ) THEN
                   ...
                   And cd_wkld_stage_pers_role='PR'; -- STGAP00014579: only update workload id unit for PR
                   -- STGAP00009841 - VVO:
       ...
       SET s.ID_UNIT = :NEW.ID_UNIT
             WHERE s.ID_STAGE IN ( SELECT w.ID_WKLD_STAGE
                 FROM workload w
           WHERE w.ID_WKLD_PERSON = :NEW.ID_PERSON 
           And cd_wkld_stage_pers_role='PR'); -- STGAP00014579: only update stage id unit for PR
       -- End STGAP00009841
                END IF;
        END IF;
...
--STGAP00014585 - DBCR: Insert SFY'10 FOSTER_CARE_RATE information

--This dbcr copies SFY'09 rates to SFY'10 since we have not been informed of any changes.

--This has already been executed in production via data fix; however the DBCR is needed for data to be added across all environments.  The script uses a where not exists clause to prevent entry of duplicate data.

--Without this data null pointer errors occur on Payment of Care due to missing rate information for the current fiscal year.

INSERT INTO CAPS.FOSTER_CARE_RATE FCR (FCR.ID_FCARE_RATE,FCR.DT_FCARE_RT_END, FCR.DT_FCARE_RT_START, 
FCR.AMT_FCARE_RT_UNIT_RATE,FCR.CD_FCARE_RATE_SERVICE,FCR.CD_AGE_RANGE) SELECT 0,TO_DATE('06/30/2010','MM/DD/YYYY'),
TO_DATE('07/01/2009','MM/DD/YYYY'), FCR2.AMT_FCARE_RT_UNIT_RATE,FCR2.CD_FCARE_RATE_SERVICE,FCR2.CD_AGE_RANGE FROM
CAPS.FOSTER_CARE_RATE FCR2 WHERE FCR2.DT_FCARE_RT_END = TO_DATE('06/30/2009','MM/DD/YYYY')
AND NOT EXISTS (SELECT FCR3.ID_FCARE_RATE FROM CAPS.FOSTER_CARE_RATE FCR3
WHERE FCR3.CD_FCARE_RATE_SERVICE=FCR2.CD_FCARE_RATE_SERVICE AND
FCR3.CD_AGE_RANGE =FCR2.CD_AGE_RANGE AND
FCR3.DT_FCARE_RT_END > TO_DATE('06/30/2009','MM/DD/YYYY'));

insert into caps.schema_version (id_schema_version, application_version, comments)
                        values (490, 'SacwisRev3', 'Release 3.13 - DBCRs 14585,14579');
