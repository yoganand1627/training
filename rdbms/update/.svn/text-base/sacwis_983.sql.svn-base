--STGAP00016099 - Release(4.2) MR-075  Update TIBR_CAPS_RESOURCE trigger

--MR-075 Update TIBR_CAPS_RESOURCE trigger to set Reimbursability status on same day as approved closure and update any necessary resource history record to avoid period overlaps.
  ...
  IF NVL(:NEW.IND_RSRC_WRITE_HIST,'N') = 'Y' THEN
    ...
    ELSIF (:NEW.CD_RSRC_FA_HOME_STATUS IN ('APP','AUN','CSD','INQ','PUN','WTL')) THEN
      ...
      ELSE
        ...
        -- Unless the new status is 'CSD' then set effective date to the current date
        IF :NEW.CD_RSRC_FA_HOME_STATUS = 'CSD' THEN
...
insert into caps.schema_version(id_schema_version,application_version,comments)
            values (984, 'SacwisRev4', 'Release 4.2 - DBCR 16099');
