--STGAP00016108 - Release(4.2) MR-075 Update TIBR_CAPS_RESOURCE trigger

--MR-075 Update TIBR_CAPS_RESOURCE trigger with column name change
...
--
  ...
  IF NVL(:NEW.IND_RSRC_WRITE_HIST,'N') = 'Y' THEN
    ...
    ELSIF (:NEW.CD_RSRC_FA_HOME_STATUS IN ('APP','AUN','CSD','INQ','PUN','WTL')) THEN
      ...
      ELSE
        ...
        -- Unless the new status is ('CSD', 'INQ', 'APP', 'WTL') then set effective date to the current date
        IF :NEW.CD_RSRC_FA_HOME_STATUS IN ('CSD', 'INQ', 'APP', 'WTL') THEN
          :NEW.DT_REIMBURSABLE_EFFECTIVE := TRUNC(SYSDATE);
        END IF;
    ...
    SET    RH.DT_REIMBURSABLE_END           = GREATEST(:NEW.DT_REIMBURSABLE_EFFECTIVE, RH.DT_REIMBURSABLE_EFFECTIVE)
  ...
  --      of RH.DT_REIMBURSABLE_EFFECTIVE or :NEW.DT_REIMBURSABLE_EFFECTIVE date.
  ...
  SET    RH.DT_RSHS_END            = :NEW.DT_LAST_UPDATE,
         RH.DT_REIMBURSABLE_END    = NVL2(RH.DT_REIMBURSABLE_EFFECTIVE, 
                                          GREATEST(RH.DT_REIMBURSABLE_EFFECTIVE, :NEW.DT_REIMBURSABLE_EFFECTIVE),  
                                          NULL)
  WHERE  RH.ID_RESOURCE            = :NEW.ID_RESOURCE
...
insert into caps.schema_version(id_schema_version,application_version,comments)
            values (992, 'SacwisRev4', 'Release 4.2 - DBCR 16108');
