--STGAP00015035 - Need a new work table and view to support complex queries in new SQR report


create table caps.report_work
(key varchar2(20),
TextParm varchar2(100),
NumberParm number(30,0),
DateParm date) tablespace data01;

grant update,insert,delete,select on caps.report_work to capson,capsbat,ops$datafix;
grant select on caps.report_work to operator;
...
WHERE
   ...
   STAGE_ONG_M.cd_stage_cnty like (select nvl(textparm,'%') from report_work where key='cd_county') AND
    ...
    STAGE_PERSON_LINK_CM_M.CD_STAGE_PERS_ROLE IN ('PR','HP') AND
    months_between(least(sysdate, last_day(to_date((select textparm from report_work where key='dt_report'), 'mm/yyyy')), NVL( STAGE_ONG_M.DT_STAGE_CLOSE , sysdate)),  STAGE_ONG_M.DT_STAGE_START ) >= 2
ORDER BY
...
insert into caps.schema_version (id_schema_version, application_version, comments)
                        values (511, 'SacwisRev3', 'Release 3.2 - DBCR 15035');
