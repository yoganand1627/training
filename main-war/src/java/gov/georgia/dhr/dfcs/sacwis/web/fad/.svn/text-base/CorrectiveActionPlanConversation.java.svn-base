 ...
 *                                          Corrective Action plans created before sealing should be View only 
...
public class CorrectiveActionPlanConversation extends BaseHiddenFieldStateConversation {
  ...
  private static final String CORRECTIVE_ACTION_TASKCD = "8092";

  private static final String CORRECTIVE_ACTION_PLAN = "Corrective Action Plan";

  private static final String DISPLAY_CORRECT_ACTION_PLAN_URL = "/fad/CorrectiveActionPlan/displayCorrectiveActionPlan";
  ...
  /**
   * This method is called by GRNDS to clear state and initially display Corrective Action Plan page.
   * 
   * @param context
   *                The GrndsExchangeContext object.
   */
  public void displayCorrectiveActionPlan_xa(GrndsExchangeContext context) {
    ...
    try {
      ...
      CaseUtility.Stage stage = null;
      if(GlobalData.getUlIdStage(request) > 0){
      stage = CaseUtility.getStage("" + GlobalData.getUlIdStage(request));
      }
      
      Date dtSealed  = null;
      if(stage != null){
        dtSealed = DateHelper.toJavaDate(stage.getDtSealed());
      }
      
      CaseUtility.Event event = null;
      if(GlobalData.getUlIdEvent(request) > 0){
        event = CaseUtility.getEvent("" + GlobalData.getUlIdEvent(request));
      }
      Date dtEventOccured = null;
      if(event != null){
       dtEventOccured = event.getDtEventOccurred();
      }
     ...
     // SMS#52234 if the user is not SAU_SEALED OR if SAU_SEALED and NoStageAccess, the Corrective Action Plan should
      // be View only if it was
     // created before the sealing date (dtSealed)
      UserProfile userProfile = getUserProfile(request);      
      if(dtEventOccured != null && dtSealed != null){
        if(DateHelper.isBefore(dtEventOccured, dtSealed) || DateHelper.isEqual(dtEventOccured, dtSealed)){
        if(!userProfile.hasRight(UserProfile.SEC_SAU_SEALED) || 
                        (userProfile.hasRight(UserProfile.SEC_SAU_SEALED) && !CaseUtility.hasStageAccess(userProfile.getUserID(), GlobalData.getUlIdStage(request)))
           ){
      ...
      if (taskCode.equals(CORRECTIVE_ACTION_TASKCD)) {
        // Check whether the Document Exists already
        nonComplianceSI.setCdNonCompliance(CodesTables.CEVNTTYP_CRA);
        ...
        if (GlobalData.getUlIdEvent(request) > 0) {
          ...
          // a stage closure approval to do, the pending Non Compliance will be
      ...
      displayCorrectiveActionPlan(context);
    } catch (Exception e) {
      GrndsTrace.msg(TRACE_TAG, 7, "General Failure: " + e.getMessage());
      handleException(e, context, "displayCorrectiveActionPlan_xa");
    }
  }

  /**
   * This method is called by GRNDS to save the Corrective Action Plan page.
   * 
   * @param context
   *                The GrndsExchangeContext object.
   */
  public void saveCorrectiveActionPlan_xa(GrndsExchangeContext context) {
    ...
    try {
      ...
      GrndsTrace.msg(TRACE_TAG, 7, "General Failure: " + e.getMessage());
      handleException(e, context, "saveCorrectiveActionPlan_xa");
    }
  }
   ...
   *                The GrndsExchangeContext object.
   */
  public void saveSubmitCorrectiveActionPlan_xa(GrndsExchangeContext context) {
    ...
    try {
      ...
      GrndsTrace.msg(TRACE_TAG, 7, "General Failure: " + e.getMessage());
      handleException(e, context, "saveSubmitCorrectiveActionPlan_xa");
    }
  }
   ...
   *                The GrndsExchangeContext object.
   */
  private void displayCorrectiveActionPlan(GrndsExchangeContext context) {
    PerformanceTrace performanceTrace = new PerformanceTrace(TRACE_TAG, "displayCorrectiveActionPlan");
    performanceTrace.enterScope();

    try {

    } catch (Exception e) {
      handleException(e, context, "displayCorrectiveActionPlan");
    }
  }
  ...
  private NonComplianceSI createNonComplianceSIFromRequestForSave(GrndsExchangeContext context) {
    ...
    try {
      ...
      // Set Date State Office Concurrence and Date CPA Concurrence to current date to system date
      ...
      ncEvent.setEventTypeCode(CodesTables.CEVNTTYP_CRA);
      ncEvent.setCaseId(GlobalData.getUlIdCase(request));
      UserProfile user = UserProfileHelper.getUserProfile(request);
      ncEvent.setPersonId(user.getUserID());
      ncEvent.setEventDescription(CORRECTIVE_ACTION_PLAN);
      ncEvent.setEventTaskCode(CORRECTIVE_ACTION_TASKCD);
