...
public class AuditLogger {
  ...
  private static final String INSERT_RECORD_SQL = "INSERT INTO PORTAL_AUDIT " +
                                                  "(ID_PORTAL_AUDIT,DT_USER_ACTION,ID_PERSON,TXT_IP_ADDRESS," +
                                                  "ID_COMMAND, NBR_MESSAGE, ID_CASE, ID_STAGE, ID_EVENT," +
                                                  "ID_USER, ID_RESOURCE, ID_PARENT_RSRC) " +
                                                  "VALUES (0,?,?,?,?,?,?,?,?,?,?,?)";
  ...
  private static void writeToDB(List<AuditRecord> insertList) {
    ...
    try {
      ...
      for (Iterator<AuditRecord> insertIterator = insertList.iterator(); insertIterator.hasNext();) {
        ...
        insertBatch.setInt(9, auditRecord.getIdUser());
        insertBatch.setInt(10, auditRecord.getIdResource());
        insertBatch.setInt(11, auditRecord.getIdParentRsrc());
