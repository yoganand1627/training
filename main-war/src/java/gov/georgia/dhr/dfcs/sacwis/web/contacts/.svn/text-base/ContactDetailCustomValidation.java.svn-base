 ...
 *  08/17/2010 bgehlot          66380: MR-072 Added new message 
 *  05/24/2011 schoi            SMS #109398: MR-086 Added a message to display an error message for Transfer Summary
 *                              if at least one person selected as Discussed/In Reference To
 *  05/24/2011 schoi            SMS #109398: MR-086 Updated Message call to MSG_DISCUSSED_REQUIRED_TRNSFR_SUMMARY 
  ...
  protected boolean validateForm() {
    ...
    if (isButtonPressed("btnSave") || isButtonPressed("btnSaveAndSubmit")) {
      ...
      if (presentationMode.equals(ContactSearchListDetailConversation.DETAIL_CONTACT)) {
        ...
        //MR-072 User selects a Purpose of Collateral without at least one person selected as Discussed/In Reference To
      ...
      // SMS #109398: MR-086
      if (presentationMode.equals(ContactSearchListDetailConversation.TRN_SUMMARY_CONTACT)) {
        // Display an error message if at least one person selected as Discussed/In Reference To
        String[] checkedDiscussedPersonsList = CheckboxHelper.getCheckedValues(request, "cbxDiscussed");
        if (checkedDiscussedPersonsList.length == 0) {
          setErrorMessage(Messages.MSG_DISCUSSED_REQUIRED_TRNSFR_SUMMARY);
        }        
      }
      // End of SMS #109398: MR-086
