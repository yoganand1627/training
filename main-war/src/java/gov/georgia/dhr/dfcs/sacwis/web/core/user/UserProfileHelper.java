...
public class UserProfileHelper {
  ...
  /** String SECURITY_QUESTION_KEY */
  public static final String SECURITY_QUESTION_KEY = TRACE_TAG + "SecurityQuestionKey";
  /** String SECURITY_ANSWER_KEY */
  public static final String SECURITY_ANSWER_KEY = TRACE_TAG + "SecurityAnswerKey";
  /** String NEW_PASSWORD_KEY */
  public static final String NEW_PASSWORD_KEY = TRACE_TAG + "NewPasswordKey";
  /** String NEW_PASSWORD_CONFIRM_KEY */
  public static final String NEW_PASSWORD_CONFIRM_KEY = TRACE_TAG + "NewPasswordConfirmKey";
  /** String LDAP baseDN */
...
//    Determine which authentication method to use.
      validateLogin_DB(userName, password);
  ...
  private static void validateLogin_DB(String userName, String password) throws SecurityException {
    //String baseDN = "ou=People,o=SACWIS,c=US";
    userName = userName == null ? "" : userName;
    password = password == null ? "" : password;
    try {
      ...
      if((userPasswd = getPortalUserPasswd(userName))!=null && userPasswd.equals(password)){
        GrndsTrace.msg(TRACE_TAG, 9, "\nAuthenticated");
      }
      else{
        GrndsTrace.msg(TRACE_TAG, 9, "\nNot Authenticated");
  ...
  public static String getPortalUserPasswd(String userName) throws SQLException {
    Connection conn = JdbcHelper.getConnection();
    PreparedStatement stmt = null;
    ResultSet result = null;
    try {
      stmt = conn.prepareStatement("Select caps.decrypt(txt_password) as txt_password from portal_user where txt_user_email = ? ");
      stmt.setString(1, userName);
      result = stmt.executeQuery();
      if(result.next()){
        return result.getString("txt_password");
      }
      return null;
    }
    catch (SQLException e) {
