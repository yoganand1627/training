...
public class AuditInit implements Initializable, Destroyable, Serializable {
    ...
    try {
      ...
      if (updateSet.size() > 0) {
        String sql = "INSERT INTO " +
                     "PORTAL_COMMAND ( NM_SERVLET, NM_CONVERSATION, NM_COMMAND, NM_BRANCH ) " +
                     "VALUES ( ?, ?, ?, ? )";
      ...
      defaultBranchStatement =
              connection.prepareStatement("INSERT INTO PORTAL_COMMAND \n" +
  ...
  private static SortedSet getDBURISet() {
    ...
    try {
      connection = JdbcHelper.getConnection();
      String sql = "SELECT ID_COMMAND, NM_SERVLET, NM_CONVERSATION, NM_COMMAND, NM_BRANCH FROM PORTAL_COMMAND";
